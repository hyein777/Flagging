...
*/
	...
	//mysqli_fetch_row 함수는 mysqli_query 를 통해 얻은 리절트(result)에서 레코드를 1개씩 리턴해주는 함수

	if($row[0]){ 
			//while문을 쓰면 코스 데이터가 다 들어온다.
            //echo는 document.write같은거로 화면에 보여주겠다는 뜻
            //$row[2]로 [2]는 테이블에서 해당 컬럼의 인덱스 값
            //만약 *이면 전체 기준으로 인덱스가 정해지	는데,
            //몇 개중에서 검색하면 그거 순서로 인덱스가 정해진다.
            // $course_data = $course_data."(start)".$row[2]."!!".$row[3]."!!".$row[4]."!!".$row[5]."!!".$row[6]."!!".$row[7]."!!";
            echo $row[3];
        }

	//종료한다.
	$db_close = mysqli_close($db_con);
	// http://belikhs.dothome.co.kr/project2_test/php/get_user_data.php?user_name=heesoo
